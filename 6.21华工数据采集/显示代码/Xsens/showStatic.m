...
subplot(311);plot(t1,DATA.data(:,2),t1,Acc_X_Noise);legend('原始x');title('加速度');ylabel('m/s2');xlabel('t/s');
subplot(312);plot(t1,DATA.data(:,3),t1,Acc_Y_Noise);legend('原始y');ylabel('m/s2');xlabel('t/s');
subplot(313);plot(t1,DATA.data(:,4),t1,Acc_Z_Noise);legend('原始z');ylabel('m/s2');xlabel('t/s');
figure(2);%角速度
...
subplot(311);plot(t1,DATA.data(:,5),t1,Gro_X_Noise);legend('原始x');title('角速度');ylabel('deg/s');xlabel('t/s');
subplot(312);plot(t1,DATA.data(:,6),t1,Gro_Y_Noise);legend('原始y');ylabel('deg/s');xlabel('t/s');
subplot(313);plot(t1,DATA.data(:,7),t1,Gro_Z_Noise);legend('原始z');ylabel('deg/s');xlabel('t/s');
figure(3);%磁力计，归一化地磁矢量
subplot(311);plot(t1,DATA.data(:,8));legend('原始x');title('磁力计');ylabel('AU');xlabel('t/s');
subplot(312);plot(t1,DATA.data(:,9));legend('原始y');ylabel('AU');xlabel('t/s');
subplot(313);plot(t1,DATA.data(:,10));legend('原始z');ylabel('AU');xlabel('t/s');
figure(4);%姿态角

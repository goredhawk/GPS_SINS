...
imuerr = imuerrset(2, 2000, 3, 150);
imu = imuadderr(trj.imu, imuerr);
davp0 = avpseterr([30;-30;20], 0.1, [10;10;15]);
...
for k=1:nn:len-nn+1
    ...
    kf = kfupdate(kf);
    if mod(t,1)==0
        posGPS = trj.avp(k1,7:9)' + davp0(7:9).*randn(3,1);  % GPS pos simulation with some white noise,高斯分布
...
meanErrLat=mean(abs(avperr(:,7)))*glv.Re;
meanErrLon=mean(abs(avperr(:,8)))*glv.Re;
meanErrHgt=mean(abs(avperr(:,9)));
